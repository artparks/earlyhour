	...
	if ( in_the_loop() ) {
		...
		foreach( $dit as $node ) {
			...
			if ($node->nodeName == '#text') {
				...
				//Make UPPERCASE words BOLD
				$node->nodeValue = preg_replace( '/\b([\p{Lu}]+)\b/u', '<b>$1</b>', $node->nodeValue );

				//Make words followed by ! bold
				$node->nodeValue = preg_replace( '/\b(\w+\!)/u', '<b>$1</b>', $node->nodeValue );

				//Make everything in quotes italic
				// first the regular quotes
				$node->nodeValue = preg_replace( '/(\"[^\"]+\")/', '<i>$1</i>', $node->nodeValue );
				// then fancy quotes
				$node->nodeValue = preg_replace( '/(\&\#8220\;[^\"]+\&\#8221\;)/', '<i>$1</i>', $node->nodeValue );

				//Make everything between () italic
				$node->nodeValue = preg_replace( '/(\([^\(\)]+\))/', '<i>$1</i>', $node->nodeValue );

				//Make everything between : and ! or ? italic
				$node->nodeValue = preg_replace( '/(?<=\:)([^\:\!\?]+[\!|\?]\S*)/', '<i>$1</i>', $node->nodeValue );

				//Make everything between start and : bold
				$node->nodeValue = preg_replace( '/(\A[^\:]+\:)/', '<b>$1</b>', $node->nodeValue );

				//Make a title with only one ? in it, bold at the left and italic at the right
				$node->nodeValue = preg_replace( '/(\A[^\?\:\!]+\?)([^\?\:\!]+)\z/', '<b>$1</b><i>$2</i>', $node->nodeValue );
			}
		}
...
function hive_wrap_images_in_figure( $content ) {
	...
	foreach ($classes as $class) {
		...
		$callback = new HiveWrapImagesInFigureCallback( $class );
...
class HiveWrapImagesInFigureCallback {
	private $class;
...
function hive_allow_skype_protocol( $protocols ){
	$protocols[] = 'skype';
	return $protocols;
}
add_filter( 'kses_allowed_protocols' , 'hive_allow_skype_protocol' ); ?>